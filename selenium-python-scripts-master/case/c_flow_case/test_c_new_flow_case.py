...
class TestNewFlow(unittest.TestCase):
    ...
    def test_new_flow(self):
        ...
        setting = Setting()
        # 创建新流程名字
        new_flow_sample.new_simple_flow_name(self.driver, gv.flow_name1)
        # 节点一命名
        ...
        # # 增加节点二
        button.add(self.driver)
        # 节点二命名
        button.node_name(self.driver, gv.nodename[1])
        # 节点二生效条件：报销/借款/申请金额大于设定值
        effect_con.money_than(self.driver)

        # 节点二通过角色选择审批人
        choose_app.role_select(self.driver)
        # 节点二可允许审批人修改单据
        setting.allow_modify(self.driver)
        # 保存
        button.save(self.driver)

        # 验证此流程名称是否正确
        actual_name = assert_flow.assert_flow_name(self.driver, gv.flow_name1)
        assertion.except_equal(gv.flow_name1, actual_name, u'审批流名称不正确', u'新建审批流成功')
