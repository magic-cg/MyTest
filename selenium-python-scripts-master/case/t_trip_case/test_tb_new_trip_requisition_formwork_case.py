...
class NewRequisitionFormWorkCase(unittest.TestCase):
    ...
    # 新建差旅申请单模板
    def test_new_trap_requisition_formwork(self):
        ...
        # 新建申请单模板
        new_requisition_fw.new_requisition_formwork(self.driver, gv.trip_requisition_model_name1)
        # 设置申请单的审批流
        new_requisition_fw.requisition_flow(self.driver)
        # 设置申请单打印模板
        new_requisition_fw.print_model(self.driver)
        # 设置申请单的申请内容
        new_requisition_fw.write_trip_direct(self.driver)
        # 保存
        new_requisition_fw.save(self.driver)
        # 验证差旅申请单模板是否已新建成功
        expense_name = get_form.get_formwork_name(self.driver, gv.trip_requisition_model_name1)
        assertion.except_equal(gv.trip_requisition_model_name1, expense_name, u'差旅申请单模板未创建成功', u'新建差旅申请单模板成功')
