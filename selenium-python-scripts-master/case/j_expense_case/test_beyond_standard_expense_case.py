...
class TestNewExpense(unittest.TestCase):
    ...
    def test_beyond_standard_expense(self):
        ...
        for i in range(0, 2):
            ...
            # 添加带有费用标准的费用类型
            new_expense.simple_fee_type(self.driver, gv.expense_sta_amount[1], gv.fee_type_name3[i])
            sleep(2)
            if i == 0:
                new_expense.flight_field(self.driver)
                sleep(1)
            if i == 1:
                # 选择时间
                new_expense.time_range(self.driver)
                sleep(1)
            # 保存费用类型
