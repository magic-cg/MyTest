...
class TestNewExpense(unittest.TestCase):
    ...
    def test_batch_expense(self):
        ...
        # 验证是否出现审批成功的吐丝
        message = operation.message_reminder(self.driver)
        assertion.except_equal(u'审批成功', message, u'审批失败', u'报销单第一个审批人批量同意')
        # 刷新页面
        self.driver.refresh()
        sleep(2)
        # 进入待审批页面
        ...
        # 验证是否出现驳回成功的吐丝
        message = operation.message_reminder(self.driver)
        assertion.except_equal(u'驳回成功', message, u'驳回失败', u'报销单第二个审批人批量驳回')
        # 重新将这两个报销单提交
        for i in range(len(collectnum)):
            # 搜索被驳回的单据
        ...
        # 验证是否出现审批成功的吐丝
        message = operation.message_reminder(self.driver)
        assertion.except_equal(u'审批成功', message, u'审批失败', u'报销单第二个审批人批量同意')
        # 刷新页面
        self.driver.refresh()
        sleep(2)
        # 进入待支付页面
        ...
        # 验证是否出现确认成功的吐丝
        message = operation.message_reminder(self.driver)
        assertion.except_equal(u'确认成功', message, u'支付失败或单据进入支付中状态', u'报销单批量支付成功')
