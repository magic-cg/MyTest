...
class TestNewExpense(unittest.TestCase):
    ...
    def test_add_verification_expense(self):
        ...
        sleep(1)
        # 关联申请单
        # operation.add_requisition(self.driver)

        # 添加核销
        nums = operation.get_loan_count(self.driver)
        operation.add_verification(self.driver, nums)
        ...
        # 获取单据支付金额
        current_money = operation.get_paid_money(self.driver)
        print current_money
        # 第一个审批人同意
        ...
        # 预算超标，是否继续
        operation.budget_excess(self.driver)
        if current_money == u'0.00':
            # 则支付金额为0
            operation.paid_zero(self.driver)
            # 验证是否出现确认成功的吐丝
            message = operation.message_reminder(self.driver)
            assertion.except_equal(u'支付成功', message, u'支付失败或单据进入支付中状态', u'报销单支付成功')
        else:
