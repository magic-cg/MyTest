...
class TestNewRequisition(unittest.TestCase):
    ...
    def test_reject_requisition(self):
        ...
        # 提交人重新选择模板，重新填写，这次是直接填写金额
        new_requisition.select_model(self.driver, gv.requisition_title, gv.requisition_model_name2)
        # 填写金额
        new_requisition.requisition_money(self.driver)
        # 再次提交申请单
        operation.submit(self.driver)
        # 选择下一审批人
        operation.select_first_person(self.driver, gv.approver)
