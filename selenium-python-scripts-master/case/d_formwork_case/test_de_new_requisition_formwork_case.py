...
class NewRequisitionFormWorkCase(unittest.TestCase):
    ...
    def test_new_requisition_formwork(self):
        ...
        # 新建申请单模板
        new_requisition_fw.new_requisition_formwork(self.driver, gv.requisition_model_name3)
        # 设置申请单的审批流
        new_requisition_fw.requisition_flow(self.driver)
        # 设置申请单打印模板
        new_requisition_fw.print_model(self.driver)
        # 选择申请关闭方式-报销金额≥申请金额时自动关闭
        new_requisition_fw.close_requisition_rule2(self.driver)
        # 保存
        new_requisition_fw.save(self.driver)
        # 验证申请单模板是否已新建成功
        expense_name = get_form.get_formwork_name(self.driver, gv.requisition_model_name3)
        assertion.except_equal(gv.requisition_model_name3, expense_name, u'申请单模板未创建成功', u'新建申请单模板成功')