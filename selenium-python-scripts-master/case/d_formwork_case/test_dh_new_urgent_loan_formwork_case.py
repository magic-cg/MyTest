...
class NewLoanFormWorkCase(unittest.TestCase):
    ...
    def test_new_loan_formwork(self):
        ...
        # 借款单模板基本设置
        new_loan_fw.new_loan_formwork(self.driver, gv.loan_model_name2)
        # 设置审批流
        new_loan_fw.loan_flow1(self.driver)
        # 设置其他项
        new_loan_fw.other(self.driver)
        # 保存
        new_loan_fw.save(self.driver)
        # 验证借款单模板是否已新建成功
        expense_name = get_form.get_formwork_name(self.driver, gv.loan_model_name2)
        assertion.except_equal(gv.loan_model_name2, expense_name, u'借款单模板未创建成功', u'新建借款单模板成功')
